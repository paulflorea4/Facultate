...
async function validateReservationForm(event) {
    ...
    // Get form elements
    const lastName = $('#last_name')[0];
    const firstName = $('#first_name')[0];
    const age = $('#age')[0];
    const gender = $('input[name="gender"]:checked')[0];
    const email = $('#email')[0];
    const phone = $('#phone')[0];
    const birthdate = $('#birthdate')[0];
    const checkIn = $('#check_in')[0];
    const checkOut = $('#check_out')[0];
    const guestCount = $('#guest_count')[0];
    const roomType = $('input[name="room_type"]:checked')[0];
    const confirmPassword = $('#confirm_password')[0];
    const notes = $('#notes')[0];
    const terms = $('#terms')[0];
    ...
    // Validate Gender
    if (!gender) {
        const genderGroup = $('input[name="gender"]').first().closest('.form-group')[0];
    ...
    // Validate Room Type
    if (!roomType) {
        const cameraGroup = $('input[name="room_type"]').first().closest('.form-group')[0];
...
$(function () {
    ...
    if ($reservationForm.length) {
        $reservationForm.on('submit', validateReservationForm);
