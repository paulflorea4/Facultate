...
-- m-n + WHERE
-- Numele,data sezonului,statisticile jucatorului care a avut cel putin 25 de puncte in sezonul 10
...
WHERE s.season_id = 10 AND ps.PPG >= 25
...
WHERE g.game_date LIKE '2023-04%';
...
-- Numele echipei si a antrenorului care au avut minim 50 de castiguri in sezonul care incepe in 2022-10-01 
SELECT t.team_name, c.coach_name, ts.wins
FROM TeamSeason ts
INNER JOIN Team t ON ts.team_id = t.team_id
INNER JOIN Season s ON ts.season_id = s.season_id
INNER JOIN Coach c ON c.team_id=t.team_id
WHERE s.year_start = '2022-10-01'
