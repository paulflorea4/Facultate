...
public class ServicesRpcProxy implements IJocServices {
    ...
    public void alegeVarianta(Jucator jucator) throws Exception {
        sendRequest(new Request(RequestType.ALEGE, jucator));
        Response response = qresponses.take();
        if (response.getType() == ResponseType.ERROR) {
            throw new Exception((String) response.getData());
        }
            ...
            try {
                switch (response.getType()) {
                    ...
                    case JOC_PORNIT:
                        ...
                        Cuvant cuvant = (Cuvant) gameData[0];
                        String mesaj = (String) gameData[1];
                        client.jocPornit(cuvant, mesaj);
                        break;
                    case INCEPE_RUNDA:
                        client.incepeRunda((String) response.getData());
                        break;
                    case ALES:
                        client.variantaAleasa((Cuvant) response.getData());
                        break;
                    default:
