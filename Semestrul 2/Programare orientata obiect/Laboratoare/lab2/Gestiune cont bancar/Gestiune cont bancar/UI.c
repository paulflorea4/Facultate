...
void printTransactions(TransactionsService* service)
{
	...
	getTransactionsService(&length, service,transactions);
	if (length!=0)
	{
...
void printFilteredTransactions(TransactionsService* service)
{
	...
	switch (filterType)
	{
		case 1:
			if (readTypeFromUser(&type) == false)
				return;
			getTransactionsFilteredByType(type, &length, service,filteredList);
			break;
		case 2:
			if (readSumFromUser(&sum) == false)
				return;
			...
			if (sumProperty != 1 && sumProperty != 2)
			{
				...
				getTransactionsFilteredBySum(sum, &length, sumProperty%2, service,filteredList);
			break;
		default:
			printf("Optiune invalida!\n");
			return;
...
void printSortedTransactions(TransactionsService* service)
{
	...
	switch (sortProperty)
	{
		case 1:
			...
			{
				getTransactionsSortedBySum(sortType % 2, service, sortedList);
			}
			break;
		case 2:
			printf("Alegeti modul de ordonare:\n");
			...
			if (sortType != 1 && sortType != 2)
			{
				...
				return;
				 ...
				 getTransactionsSortedByDay(sortType % 2, service, sortedList);
			break;
		default:
...
void runApp(TransactionsService* service)
{
	while (true)
	{
		printMenu();
